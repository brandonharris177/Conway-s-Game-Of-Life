...
function App() {
  ...
  function createGrid() {
      return new Promise (generationGrid => {
        generationGrid(Array.from({length: gridSize}).map(() => Array.from({length: gridSize}).fill(0)))
      })
  }

  function updateGrid(newGenerationGrid) {
        for (let row = 0; row < gridSize; row++) {
            for (let column = 0; column < gridSize; column++){
              if (grid[row][column] === 1) {
                newGenerationGrid[row][column] = 1
              }
            }}
      return newGenerationGrid
  }

  function countNeighbors(updatedGrid) {
    for (let row = 0; row < gridSize; row++) {
        for (let column = 0; column < gridSize; column++){
          let neighbors = 0;
  ...
  async function nGeneration() {
    let count = generation+1
    while(count < ngen) {
        const newGenerationGrid = await createGrid()
        console.log("grid1", newGenerationGrid)
        const updatedGrid = await updateGrid(newGenerationGrid)
        console.log("grid2", updatedGrid)
        const neighborGrid = await countNeighbors(updatedGrid)
        console.log("grid3", neighborGrid)
        setGrid(neighborGrid)
        count +=1
    }
    setGeneration(count)
  }
